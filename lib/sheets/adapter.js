...
// Cache ringan (per-instance serverless, TTL) khusus untuk sheet besar yang
// jarang berubah -- Employees sekarang 23 ribuan baris dan cuma di-refresh
// lewat sync HSE mingguan/manual, jadi aman di-cache beberapa menit supaya
// tidak baca ulang seluruh sheet dari Google Sheets API di setiap request.
// Events & EventAttendance dikasih cache pendek (beberapa detik) khusus untuk
// meredam lonjakan baca saat banyak orang buka halaman absensi/scan QR
// bersamaan (tiap buka halaman check-in = baca kedua sheet ini) -- staleness
// beberapa detik aman untuk kebutuhan itu. Sheet lain (Tracker, Leave, dst)
// TIDAK di-cache karena sering diedit langsung oleh user dan harus selalu fresh.
const SHEET_CACHE_TTL_MS = {
  Employees: 10 * 60 * 1000,
  Events: 5 * 1000,
  EventAttendance: 5 * 1000,
};
const sheetDataCache = new Map();
// Beberapa request bersamaan (mis. banyak orang absensi via QR sekaligus) bisa
// tiba di instance serverless yang sama sebelum cache di atas terisi -- tanpa
// ini, tiap request itu masing-masing memicu panggilan Sheets API sendiri
// (memperparah rate limit "Read requests per minute"). Request yang datang
// selagi ada fetch untuk sheet yang sama sedang berjalan cukup ikut menunggu
// promise yang sama, bukan memicu panggilan baru.
const inFlightFetches = new Map();

function getSheetCacheTtl(sheetName) {
  return SHEET_CACHE_TTL_MS[sheetName] || 0;
}

export function invalidateSheetDataCache_(sheetName) {
...
async function fetchAllValues(sheetName) {
  ...
  if (ttl > 0) {
    ...
    const pending = inFlightFetches.get(sheetName);
    if (pending) {
      return pending;
    }
  }
  ...
  const fetchPromise = (async () => {
    ...
    try {
      ...
      if (ttl > 0) {
        sheetDataCache.set(sheetName, { values, expiresAt: Date.now() + ttl });
      }
  ...
  if (ttl > 0) {
    inFlightFetches.set(sheetName, fetchPromise);
    fetchPromise.finally(() => inFlightFetches.delete(sheetName));
  }
...
export class SheetHandle {
  ...
  invalidateCache() {
    this._valuesCache = null;
    invalidateSheetDataCache_(this.name);
  }
