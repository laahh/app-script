...
// AWS Lambda (dipakai Vercel di balik layar) meng-set process.env.TZ ke
// ":UTC" secara default -- ini BUKAN nama timezone IANA yang valid dan
// bikin date-fns-tz gagal format (RangeError: Invalid time value).
// Env var TZ kita sendiri (mis. TZ=Asia/Jakarta) kalau belum di-set di
// Vercel dashboard, jangan sampai ketiban default Lambda itu.
const rawTz = process.env.TZ || "";
const TZ = rawTz && rawTz !== ":UTC" ? rawTz : "Asia/Jakarta";
...
export async function installPortalSchedulerTrigger() {
  ...
  return {
    installed: true,
    message:
      "Vercel Cron aktif (sekali sehari via vercel.json). Pastikan CRON_SECRET sudah di-set.",
  };
}
